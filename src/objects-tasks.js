...
function shallowCopy(obj) {
  ...
  arr.forEach((item) => {
    if (typeof item === 'object') isDeep = true;
  });
  if (isDeep === 0) return Object.assign(obj);
...
function mergeObjects(objects) {
  const matrix = [];
  objects.forEach((item) => matrix.push(Object.entries(item)));
  const newObj = {};
  matrix.flat().forEach((item) => {
    const [letter, number] = item;
    if (newObj[letter]) newObj[letter] += number;
    else newObj[letter] = number;
  });
  return newObj;
}
...
function removeProperties(obj, keys) {
  ...
  keys.forEach((item) => delete myObj[item]);
  return myObj;
}
...
function makeWord(lettersObject) {
  const arr = [];
  Object.keys(lettersObject).forEach((item) => {
    lettersObject[item].forEach((num) => {
      arr[num] = item;
    });
  });
  return arr.join('');
...
function sellTickets(queue) {
  ...
  let hightCash = 0;
  queue.forEach((item) => {
    if (item === 25) twentyFive += 25;
    else hightCash += item;
  });
  return twentyFive >= hightCash;
}
...
function sortCitiesArray(/* arr */) {
  throw new Error('Not implemented');
  // const brr = arr.filter((item) => item);
  // brr.sort((item1, item2) => {
  //   if (item1.country < item1.country) return 1;
  //   if (item1.country > item1.country) return -1;
